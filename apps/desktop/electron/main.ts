...
// electron-builder derives from the build `appId` (com.nastechairesearch.nastech) —
// keep this string in sync with package.json `build.appId`. macOS/Linux don't
// need this, so gate it on Windows. (Fixes: desktop approval/turn notifications
// never firing on Windows.)
if (IS_WINDOWS) {
  app.setAppUserModelId('com.nastechairesearch.nastech')
}
...
app.setAboutPanelOptions({
  ...
  copyright: 'Copyright © 2026 NasTech'
})
...
function resolveNasTechBackend(backendArgs) {
  ...
  if (process.env.NASTECH_DESKTOP_IGNORE_EXISTING !== '1') {
    ...
    const nastechOverride = process.env.NASTECH_DESKTOP_HERMES
    ...
    if (nastechCommand) {
      ...
      // NASTECH_DESKTOP_HERMES is an explicit deployment override (used by
...
// NasTech) instead of a static session token. The auth model is
...
// convention (nastech_cli/auth.py DEFAULT_NOUS_PORTAL_URL + the same env names)
// so a single override flips every NasTech surface to the same portal.
const DEFAULT_NOUS_PORTAL_URL = 'https://portal.nastechairesearch.com'

function resolvePortalBaseUrl() {
  const raw = process.env.NASTECH_PORTAL_BASE_URL || process.env.NOUS_PORTAL_BASE_URL || DEFAULT_NOUS_PORTAL_URL
...
async function probeRemoteAuthMode(rawUrl) {
  ...
  if (authRequired) {
    ...
    // button can read "Sign in with NasTech" instead of a generic
