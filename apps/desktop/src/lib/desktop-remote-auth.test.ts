...
describe('deriveRemoteAuthProviderShape', () => {
  ...
  it('keeps OAuth copy for redirect providers and mixed deployments', () => {
    expect(
      deriveRemoteAuthProviderShape([{ name: 'nous', displayName: 'NasTech', supportsPassword: false }])
    ).toEqual({
      isPassword: false,
    ...
    expect(
      deriveRemoteAuthProviderShape([
        ...
        { name: 'nous', displayName: 'NasTech', supportsPassword: false }
      ])
    ).toEqual({
      isPassword: false,
      providerLabel: 'Username & Password / NasTech'
    })
  })
