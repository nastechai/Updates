...
If the file is empty, whitespace-only, or cannot be read, NasTech falls back to a built-in default identity ("You are NasTech Agent, an intelligent AI assistant created by NasTech..."). This fallback also applies when `skip_context_files` is set (e.g., in subagent/delegation contexts).
