...
[Nous Portal](https://portal.nastechairesearch.com) 是 NasTech 的统一订阅网关，也是**运行 NasTech Agent 的推荐方式**。一次 OAuth 登录，即可替代原本需要手动配置的各模型厂商独立账号、API 密钥和计费关系。
...
这条命令会完成 Portal OAuth 认证，让你选择一个 Nous 模型，在 `config.yaml` 中将 Nous 设为推理提供商，并开启 Tool Gateway。完成后即可立即运行 `nastech chat`。
...
同一订阅还解锁了 [Tool Gateway](/user-guide/features/tool-gateway)，将 NasTech Agent 的工具调用路由至 Nous 托管的基础设施。五个后端，一次登录：
...
NasTech 自家的 **NasTech 4** 系列（NasTech-4-70B、NasTech-4-405B）通过 Portal 提供，享有大幅折扣。这些是**前沿混合推理对话模型**——在数学、科学、指令遵循、schema 遵从、角色扮演和长文写作方面表现出色。
...
Portal 自身的[模型信息页](https://portal.nastechairesearch.com/info)也有相同警告，因此这不是 NasTech 侧的主观意见——这是 NasTech 的官方指导。
...
4. 在 `~/.nastech/config.yaml` 中将 Nous 设为推理提供商（当你选择模型时）
5. 开启 Tool Gateway（网页、图像、TTS、浏览器路由）
6. 返回终端，即可运行 `nastech chat`

如果还没有订阅，请先在 [portal.nastechairesearch.com/manage-subscription](https://portal.nastechairesearch.com/manage-subscription) 注册。
...
```bash
nastech portal            # 登录 Nous Portal 并完成配置（一键引导）
...
`nastech portal`（不带子命令）是 `nastech auth add nous --type oauth` 的易记别名——它会登录、让你选择 Nous 模型、把 Nous 设为推理服务商，并提供 Tool Gateway 启用选项（与 `nastech setup --portal` 等价，与首次快速设置走的是同一套 Nous 流程）。
...
如果你已有 Browserbase 账号并希望继续使用，同时通过 Nous 路由网页搜索和图像生成，这是支持的。使用 `nastech tools` 为每个工具单独选择后端：
...
- **网页端：** [portal.nastechairesearch.com/manage-subscription](https://portal.nastechairesearch.com/manage-subscription)
...
如果 Portal 使 refresh token 失效（修改密码、手动撤销、会话过期），失效的 refresh token 会被**本地隔离**，NasTech 停止重放该令牌，你不会看到一连串相同的 401 错误。下一次调用会显示清晰的"需要重新认证"提示。运行 `nastech auth add nous` 重新登录；隔离状态在下次成功登录时自动清除。
...
或使用 `nastech model` 重新选择 Nous Portal。

### 会话中途收到"需要重新认证"提示

你的 Portal refresh token 已失效（修改密码、手动撤销或会话过期）。运行 `nastech auth add nous`，下一次请求将使用新凭证。旧令牌的隔离状态在成功重新登录后自动清除。
...
先检查 `nastech portal info`——如果显示你正在使用其他提供商（`Model: currently openrouter` 而非 `using Nous as inference provider`），说明本地配置已偏离。运行 `nastech model`，选择 Nous Portal，下一次请求将通过你的订阅路由。
