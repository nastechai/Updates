---
sidebar_position: 1
title: "通过 Nous Portal 运行 NasTech Agent"
description: "完整操作指南：订阅、配置、切换模型、启用 gateway 工具并验证路由"
---

# 通过 Nous Portal 运行 NasTech Agent

本指南带你从头到尾完成在 [Nous Portal](https://portal.nastechairesearch.com) 订阅下运行 NasTech Agent 的全过程——从注册账号到验证每个工具的路由是否正确。如果你只想了解 Portal 的概述及订阅内容，请参阅 [Nous Portal 集成页面](/integrations/nous-portal)。本页是操作步骤脚本。
...
打开 [portal.nastechairesearch.com/manage-subscription](https://portal.nastechairesearch.com/manage-subscription)，注册并选择一个套餐。
...
3. 在 `~/.nastech/config.yaml` 中设置 `model.provider: nous`
...
Portal 的[信息页面](https://portal.nastechairesearch.com/info)也有此说明——这是 Nous 官方指导，并非仅代表 NasTech 一方的意见。
...
```bash
nastech auth logout nous       # 清除本地 refresh token
