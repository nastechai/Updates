...
_NOUS_DEFAULT_BASE_URL = "https://inference-api.nastechairesearch.com/v1"
...
def _nous_min_key_ttl_seconds() -> int:
    try:
        return max(60, int(os.getenv("NASTECH_NOUS_MIN_KEY_TTL_SECONDS", "1800")))
...
def _nous_api_key(provider: dict) -> str:
    ...
    from nastech_cli.auth import _nous_invoke_jwt_is_usable
...
def _build_call_kwargs(
    ...
    if max_tokens is not None:
        ...
        if _provider_norm in {"nous", "nous-portal", "nastechairesearch"}:
            from nastech_cli.providers import nous_api_mode
    ...
    if _provider_for_portal in {"nous", "nous-portal", "nastechairesearch"}:
    ...
    if reasoning_config and isinstance(reasoning_config, dict):
        ...
        effective_base = base_url or ""
        _nous_on_messages = False
        if provider_norm in {"nous", "nous-portal", "nastechairesearch"}:
            from nastech_cli.providers import nous_api_mode
