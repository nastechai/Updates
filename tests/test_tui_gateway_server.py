...
def test_resolve_model_strips_config_model(monkeypatch):
    ...
    monkeypatch.setattr(
        server, "_load_cfg", lambda: {"model": {"default": " nous/nastech-test "}}
    )

    assert server._resolve_model() == "nous/nastech-test"
...
def test_startup_runtime_uses_tui_provider_env(monkeypatch):
    monkeypatch.setenv("NASTECH_MODEL", "nous/nastech-test")
    monkeypatch.setenv("NASTECH_TUI_PROVIDER", "nous")
    monkeypatch.delenv("NASTECH_INFERENCE_PROVIDER", raising=False)

    assert server._resolve_startup_runtime() == ("nous/nastech-test", "nous")
...
def test_startup_runtime_does_not_treat_inference_provider_as_explicit(monkeypatch):
    ...
    assert server._resolve_startup_runtime() == ("nous/nastech-test", None)
...
def test_billing_error_serialization_preserves_server_code(
    ...
    import nastech_cli.nous_billing as nb
