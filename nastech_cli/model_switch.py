...
_NASTECH_MODEL_WARNING = (
    "NasTech NasTech 3 & 4 models are NOT agentic and are not designed "
    "for use with NasTech Agent. They lack the tool-calling capabilities "
    "required for agent workflows. Consider using an agentic model instead "
    "(Claude, GPT, Gemini, DeepSeek, etc.)."
)

# Match only the real NasTech NasTech 3 / NasTech 4 chat families.
...
_NOUS_NASTECH_NON_AGENTIC_RE = re.compile(
    r"(?:^|[/:])nastech[-_ ]?[34](?:[-_.:]|$)",
    re.IGNORECASE,
)
...
# ---------------------------------------------------------------------------
def is_nous_nastech_non_agentic(model_name: str) -> bool:
    ...
    if not model_name:
        return False
    return bool(_NOUS_NASTECH_NON_AGENTIC_RE.search(model_name))
...
def _check_nastech_model_warning(model_name: str) -> str:
    ...
    if is_nous_nastech_non_agentic(model_name):
...
def switch_model(
    ...
    if target_provider in {"nous", "nous-portal", "nastechairesearch"}:
    ...
    for pid, overlay in NASTECH_OVERLAYS.items():
        ...
        # reflects the active region (eu.*, ap.*) not the static us.* list.
            ...
            try:
                ...
                from nastech_cli.auth import get_provider_auth_state as _nous_state
