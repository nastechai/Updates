...
# `NASTECH_DESKTOP_HERMES` override env var, so the desktop's resolver
...
stdenv.mkDerivation {
  ...
  installPhase = ''
    ...
    # NASTECH_DESKTOP_HERMES to the absolute path of the nix-built `nastech`
    ...
    makeWrapper ${lib.getExe electron} $out/bin/nastech-desktop \
      ...
      --set NASTECH_DESKTOP_HERMES "${lib.getExe nastechAgent}" \
